...
```mermaid
flowchart TD
    A["CAN problem"] --> B["Software / protocol"]
    A --> C["Host interface"]
    A --> D["Device state"]
    A --> E["Physical layer"]

    B --> B1["application, ID map, encoding"]
    C --> C1["adapter, driver, bitrate"]
    D --> D1["power, mode, enable"]
    E --> E1["wiring, termination, signal"]
```
...
2. If expected frames are visible, check software and protocol assumptions first.
3. If frames are missing, check interface state, bitrate, and device power/mode.
4. If configuration looks right, check wiring, termination, and error counters.
5. Use an oscilloscope only when the simpler checks cannot explain the failure.
```

This is not the only possible debugging sequence. It is a conservative one. It tries to avoid unnecessary hardware disassembly by using observable evidence first.
...
| Pattern            | What to expect in `candump`                      | First action                                  |
| ------------------ | ------------------------------------------------ | --------------------------------------------- |
| Periodic broadcast | Frames appear without a host command.            | Listen first.                                 |
| Query-response     | Nothing may appear until the host sends a query. | Send a status or parameter query.             |
| Command-response   | Feedback may appear only after a valid command.  | Send the required command or enable sequence. |
...
```text
DOWN:
  bring the interface up with the expected bitrate

ERROR-ACTIVE:
  interface state is likely OK

ERROR-WARNING / ERROR-PASSIVE:
  check whether RX or TX error counters are increasing

BUS-OFF:
  restart the interface, then fix the bus condition that caused it
```
...
```text
Host bitrate == device bitrate:
  frames can be decoded

Host bitrate != device bitrate:
  expect no valid frames or rapidly increasing errors
```
...
Wrong firmware or boot mode:
  heartbeat only, bootloader IDs, or no response

Not enabled:
  command frames may be visible but ignored

Correct mode and enabled:
  device should communicate according to its protocol
```
...
Use the counter direction to choose the next check:

```text
TX errors:
  check ACK, bitrate, termination, wiring, and whether another active node is present

RX errors:
  check bitrate, sample point, waveform quality, noise, and signal integrity
```
