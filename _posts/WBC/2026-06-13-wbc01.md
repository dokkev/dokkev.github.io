...
The solver should not know about CAN packet encoding.
...
This series is not about proposing a new WBC formulation.

It is about implementing a WBC-oriented control architecture that can be deployed on real robots.
...
WBC02:
  Representing floating-base robots in code

WBC03:
  RobotSystem API and rigid-body dynamics interface

WBC04:
  What is a Task object?

WBC05:
  Contacts, constraints, and force representation

WBC06:
  IHWBC-style inverse dynamics QP

WBC07:
  Solver output versus actuator command

WBC08:
  FSM-based control architecture for ROS 2 control

WBC09:
  How learning-based planners or policies can plug into the same stack
```

The next post starts with the most basic issue: robot representation.

For a fixed-base manipulator, it is often acceptable to think:

```text
q, q̇, and τ all live in joint space
```

For a floating-base robot, this breaks immediately.

We need to answer:

```text
Why can q and v have different dimensions?
Why is base orientation often stored as a quaternion?
Why is angular velocity 3D while quaternion orientation is 4D?
Why does Pinocchio use generalized velocity v instead of simply q̇?
Why does τ live in actuator space rather than configuration space?
```

If these representations are not clear, every later layer becomes fragile: tasks, contacts, solvers, command adapters, and hardware interfaces.
